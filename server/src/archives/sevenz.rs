...
impl ArchiveReader for SevenZHandler {
    async fn list_files(&self) -> Result<Vec<ArchiveEntry>> {
        ...
        let entries = archive
            .entries()
            .await
            .map_err(|e| anyhow!("Failed to list entries: {}", e))?;
    ...
    async fn open_file(&self, path: &str) -> Result<Box<dyn AsyncSeekableReader>> {
        ...
        if let Some(p) = &self.path {
            ...
            // Find index and size
            let index = archive
                .get_entry_index(&path_string)
                .await
                .map_err(|e| anyhow!("Failed to find file: {}", e))?;
            let idx = index.ok_or_else(|| anyhow!("File not found in archive: {}", path_string))?;
            let file_size = archive
                .get_entry_size(idx)
                .await
